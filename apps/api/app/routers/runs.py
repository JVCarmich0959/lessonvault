...
def get_runs(lesson_id: uuid.UUID, user: CurrentUser, db: Session = Depends(get_db)):
...
@router.post("/lesson-plans/{lesson_id}/runs", response_model=LessonRunOut, dependencies=[Depends(require_workspace_roles("owner"))])
def post_run(lesson_id: uuid.UUID, payload: LessonRunCreate, user: CurrentUser, db: Session = Depends(get_db)):
    ws = get_or_create_default_workspace(db, user)
    lesson = get_lesson_plan(db, lesson_id, ws.id)
    if not lesson:
        raise HTTPException(status_code=404, detail="Lesson plan not found")
...
def get_one_run(run_id: uuid.UUID, user: CurrentUser, db: Session = Depends(get_db)):
...
@router.patch("/runs/{run_id}", response_model=LessonRunOut, dependencies=[Depends(require_workspace_roles("owner"))])
def patch_run(run_id: uuid.UUID, payload: LessonRunUpdate, user: CurrentUser, db: Session = Depends(get_db)):
    ws = get_or_create_default_workspace(db, user)
    run = get_run(db, run_id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
...
def get_run_reflection(run_id: uuid.UUID, user: CurrentUser, db: Session = Depends(get_db)):
    ws = get_or_create_default_workspace(db, user)
    run = get_run(db, run_id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
...
@router.post("/runs/{run_id}/reflection", response_model=ReflectionOut, dependencies=[Depends(require_workspace_roles("owner"))])
def post_run_reflection(run_id: uuid.UUID, payload: ReflectionCreate, user: CurrentUser, db: Session = Depends(get_db)):
    ws = get_or_create_default_workspace(db, user)
    run = get_run(db, run_id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
...
@router.patch("/runs/{run_id}/reflection", response_model=ReflectionOut, dependencies=[Depends(require_workspace_roles("owner"))])
def patch_run_reflection(run_id: uuid.UUID, payload: ReflectionUpdate, user: CurrentUser, db: Session = Depends(get_db)):
    ws = get_or_create_default_workspace(db, user)
    run = get_run(db, run_id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
